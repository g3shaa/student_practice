...
namespace PointandRectangle
{
    public partial class FormPointAndRectangle : Form
    {
        ...
        private void Draw()
        {
            ...
            // Показва местоположението на точката : Inside / Border / Outside
            DisplayPointLocation(x1, y1, x2, y2, x, y);
            /* Изчислете коефициента на мащаба (съотношението) за диаграмата, съдържаща правоъгълник и точка, за да се поберат добре в кутията на картината */
            ...
            using (var g = Graphics.FromImage(pictureBox.Image))
            {
                ...
                g.Clear(Color.White);
                // Начертава правоъгълника (scaled to the picturebox size)
                var pen = new Pen(Color.Blue, 3);
                g.DrawRectangle(pen, rect);
                // Начертава точката (scaled to the picturebox size)
